...
class MapMaker
  ...
  def render_color_scaled_map(property,shapefile,range = 0..100)
    ...
    map.render_to_file("public/map/#{@room}_#{property}.png")
  end
  ...
  def render_crops(shapefile)
    ...
    map.zoom_to_box(map.layers.first.envelope)
    map.render_to_file("public/map/#{@room}_crop.png")
  end
  ...
  def render_farms(shapefile, data)
    ...
    map.zoom_to_box(map.layers.first.envelope)
    map.render_to_file("public/map/#{@room}_farms.png")
  end
  ...
  def field_shapefile(data)
    ...
    %w{shp shx dbf png}.each{|ext|
      File.delete "public/map/#{@room}_places.#{ext}" if File.exist? "public/map/#{@room}_places.#{ext}" 
      File.delete "public/map/#{@room}_places_points.#{ext}" if File.exist?("public/map/#{@room}_places_points.#{ext}")
    }

    shpfile = Shp4r::ShpFile.create("public/map/#{@room}_places.shp",Shp4r::ShpType::POLYLINE,dbf_fields)
    ...
    shp2 = Shp4r::ShpFile.create("public/map/#{@room}_places_points.shp",Shp4r::ShpType::POINT,dbf_fields)
  ...
  def farm_shapefile(data)
    ...
    %w{shp shx dbf png}.each{|ext|
      File.delete "public/map/#{@room}_farms.#{ext}" if File.exist? "public/map/#{@room}_farms.#{ext}" 
      File.delete "public/map/#{@room}_farms_points.#{ext}" if File.exist?("public/map/#{@room}_farms_points.#{ext}")
    }

    shpfile = Shp4r::ShpFile.create("public/map/#{@room}_farms.shp",Shp4r::ShpType::POLYLINE,dbf_fields)
    shpfile.transaction do |tr|
      polys.each_with_index{|poly,i|
    ...
    shpfile = Shp4r::ShpFile.create("public/map/#{@room}_farms_points.shp",Shp4r::ShpType::POINT,dbf_fields)
  ...
  def render_fields(data)
    ...
    render_color_scaled_map('GBI',"public/map/#{@room}_places.shp")
    render_color_scaled_map('SOC',"public/map/#{@room}_places.shp",0..190)
    render_crops("public/map/#{@room}_places.shp")
    render_farms("public/map/#{@room}_farms.shp",data)
  end
...
if ARGV[0]
  if File.exist?(ARGV[0])
    ...
    field_data = JSON.parse(IO.read(File.dirname(__FILE__) + '/../public/map/test.json'))
  end
  ...
  map_str = m.one_xml_map(field_data,"public/map/#{@room}_farms.shp","public/map/#{@room}_places.shp")
  open("#{@room}.xml",'w'){|f| f.write map_str }

  # `eog public/map/noNameRoom_farms.png`
